...
class Sendtoeworks_dp_utility_c extends Project_library
{
    ...
    public function index()
    {
        ...
        $this->load->view('dep/utility/report_index_view', $data);
    }
    ...
    public function getProjectListGrid()
    {
        ...
        $strSelect .= 'select 
			...
			IF(a.WORK_STATUS<5, "<span class=\'cus-eye\'></span>",
                     ...
                     "<span class=\'cus-cancel\'></span>")) as MY_PROJECT_STATUSx,
                     a.WORK_STATUS as MY_PROJECT_STATUS, 
			CONCAT(a.SESSION_START_YEAR, "-",a.SESSION_END_YEAR)AS TARGET_LOCK_SESSION, MONTHLY_LOCK,SETUP_LOCK
            from( ';
        $strSelect .= 'SELECT DISTINCT ps.PROJECT_SETUP_ID, ps.AA_DATE as  PROJECT_START_DATE, ps.PROJECT_CODE, ps.PARENT_PROJECT_ID, 
				CONCAT(ps.WORK_NAME,"<br />", ps.WORK_NAME_HINDI)as PROJECT_NAME,
				CONCAT(p.PROJECT_NAME,"<br />", p.PROJECT_NAME_HINDI)as PARENT_PROJECT_NAME,SETUP_LOCK,
        ...
        */
        ...
        if ($objFilter->TOTAL_RECORDS) {
			foreach ($objFilter->RESULT as $row) {
                ...
                array_push($fieldValues, '"' . addslashes($row->PROJECT_SUB_TYPE) . '"');                
				...
				array_push($fieldValues, '"' . addslashes($row->MONTHLY_LOCK) . '"');
				array_push(
					$fieldValues,
					'"' . addslashes(
						getButton(
							'Send Data',
							'sendData(' . $row->PROJECT_SETUP_ID . ')',
							4,
				...
				array_push($fieldValues, '"' . addslashes($row->PARENT_PROJECT_ID) . '"');
    ...
    public function showProjectDataBox(){
        ...
        $eworkData = $this->dep__m_target->getEWorksDetailsDeposit($PROJECT_SETUP_ID);
		...
		if(!IS_LOCAL_SERVER){
        	...
        	// below code commneted on 17-06-2020 , un comment before live 
