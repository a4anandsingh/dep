...
<title>Deposit - Deposit Project Setup - WRD MIS</title>
...
<div id="content_wrapper">
    ...
    <div style="width:100%;float:left" align="center">
        <a href="<?php echo base_url();?>documents/help/deposit_promon_project_setup.pdf" 
            class="btn btn-primary" target="_blank">
            <span class="cus-page-white-acrobat"></span>
            Download Promon - Deposit Promon Project Setup Form (PDF File)
        </a>
        <?php echo $validMsg; ?>
    </div>
    <div style="width:45%;float:left;padding-bottom:5px;padding-top:5px" align="center">
        <table id="projectList"></table> 
        <div id="projectListPager"></div>
    </div>
    <div style="width:45%;float:left;padding-bottom:5px;padding-top:5px; padding-left: 8px;" align="center">
        <table id="projectDepList"></table>
        <div id="projectDepListPager"></div>
    </div>
    <div style="width:100%;float:left;padding-top:10px" align="center">
        <table id="depProjectList"></table>
        <div id="depProjectListPager"></div>
    </div>
    <div style="width:100%;float:left;padding-top:10px" align="center">
        <table id="depProjectCList"></table>
        <div id="depProjectCListPager"></div>
    </div>
    <div style="width:100%;float:left;padding-top:10px" align="center">
...
function refreshSearch(){
	...
	jQuery("#depProjectCList").setGridParam({postData:pdata, page:1}).trigger('reloadGrid');
	jQuery("#dprojectList").setGridParam({postData:pdata, page:1}).trigger('reloadGrid');
}
//
	...
	if(mode==BUTTON_DELETE){
		...
		if(mode==BUTTON_ADD_NEW){
            ...
            if(selectedRecordID>0){
                ...
                var showTitle = 'Add Deposit Project';
			...
			if(selectedRecordID>0){
				...
				var showTitle = 'Edit Deposit Projects';
				//alert(id);
				//show_project_modalBox(id, ret.PROJECT_ID);
				showModalBox('modalBox', 'showProjectSetupEntryBox', data, showTitle, 'showProjectData', true, false);
				//alert("Edit"+data['PROJECT_ID']);
...
function addNewDepositProject(parentProjectId, flag){
    //console.log(mode);
    var data = {'PARENT_PROJECT_ID':parentProjectId,'PROJECT_SETUP_ID':0,'PROJECT_SETUP_FLAG':flag};
    var showTitle = 'Add Deposit Project';
    showModalBox('modalBox', 'showProjectSetupEntryBox', data, showTitle, 'showProjectData', true, false);
}
...
function gridReload(mode){
    ...
    $("#dprojectList").jqGrid('setGridParam',gparam).trigger("reloadGrid");
    $("#depProjectCList").jqGrid('setGridParam',gparam).trigger("reloadGrid");
}
