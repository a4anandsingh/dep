                        ...
                        if( $arrColumn['NAME']=='IP_TOTAL' ){
                            ...
                            foreach ($arrBlockIps as $k => $v) {
                                ...
                                echo $targetBlockData[$arrColumn['TARGET_DATE']][$k]->KHARIF . '</td>
                                    ...
                                    <td class="ui-widget-content" align="center">' . (intVal($targetBlockData[$arrColumn['TARGET_DATE']][$k]->KHARIF)+intVal($targetBlockData[$arrColumn['TARGET_DATE']][$k]->RABI)).'</td>';
                                ...
                                echo '<td class="ui-widget-content" align="center">'.(intVal($targetBlockData[$arrColumn['TARGET_DATE']][$k]->RABI_RESTORED)+intVal($targetBlockData[$arrColumn['TARGET_DATE']][$k]->KHARIF_RESTORED)).' ';

                                $subTotalKharif += intVal($targetBlockData[$arrColumn['TARGET_DATE']][$k]->KHARIF);
                                $subTotalRabi += intVal($targetBlockData[$arrColumn['TARGET_DATE']][$k]->RABI);
                                //$subTotalIp += $targetBlockData[$arrColumn['TARGET_DATE']][$k]->IP_TOTAL;
                                $subTotalIp += (intVal($targetBlockData[$arrColumn['TARGET_DATE']][$k]->KHARIF)+intVal($targetBlockData[$arrColumn['TARGET_DATE']][$k]->RABI));

                                $subTotalKharifRestored += intVal($targetBlockData[$arrColumn['TARGET_DATE']][$k]->KHARIF_RESTORED);
                                $subTotalRabiRestored += intVal($targetBlockData[$arrColumn['TARGET_DATE']][$k]->RABI_RESTORED);
                                //$subTotalIp += $targetBlockData[$arrColumn['TARGET_DATE']][$k]->IP_TOTAL;
                                $subTotalIpRestored += (intVal($targetBlockData[$arrColumn['TARGET_DATE']][$k]->KHARIF_RESTORED)+intVal($targetBlockData[$arrColumn['TARGET_DATE']][$k]->RABI_RESTORED));
                                if($cnt == 1) {
                                    echo '</td></tr>';
                                }
                            }
